    ...
    <article class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mx-12 mt-4">
        <?php if (!empty($data['projects'])): ?>
            <?php foreach ($data['projects'] as $project): ?>
                ...
                <!-- CARD PROJECT START -->
                <a href="/projects/detail/<?= $project['id']; ?>" class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col justify-between hover:translate-y-1 hover:shadow-md transition-all" data-id="<?= $project['id']; ?>">
                    <div>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="flex items-center -space-x-2">
                            <?php
                            if (!empty($project['member_avatars'])) {
                                $avatars = explode(',', $project['member_avatars']);
                                $total = count($avatars);

                                $display_avatars = array_slice($avatars, 0, 5);

                                foreach ($display_avatars as $avatar): ?>
                                    <img src="/assets/uploads/avatars/<?= ($avatar) ? $avatar : 'default-avatar.png' ?>" class="w-7 h-7 rounded-full border-2 border-white object-cover" onerror="this.src='/assets/images/default-profile.jpg'">
                                <?php endforeach;

                                if ($total > 5): ?>
                                    <div class="w-7 h-7 rounded-full bg-gray-200 border-2 border-white flex items-center justify-center text-[10px] text-gray-600 font-bold">
                                        +<?= $total - 5 ?>
                                    </div>
                                <?php endif;

                            } else {
                                echo '<p class="text-[10px] text-gray-400 italic">No members yet</p>';
                            }
                            ?>
                        </div>
                    </div>
